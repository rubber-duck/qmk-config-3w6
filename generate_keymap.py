#!/usr/bin/env python3
"""Generate QMK keymap.c for 3w6 from Totem-style layer definitions.

The 3w6 is a split 3x5+3 keyboard (36 keys). No side keys.
Layers are defined as grids:
  - "full": 4 rows. Rows 0-2 have 10 keys (5L + 5R). Row 3 has 6 thumb keys (3L + 3R).
  - "mirrored": 4 rows. Rows 0-2 have 5 keys (left only, mirrored to right). Row 3 has 10 cols (2 empty + 3L + 3R + 2 empty).
"""


def format_layer(name, layout_macro, grid):
    """Format a full layer. Grid is 4x10: thumb row has "" in unused positions.
    Col 4 gets +6 padding (gap between halves), others +2.
    Commas separate all keys; last key of last row has no comma."""
    cw = [max(len(grid[r][c]) for r in range(4)) for c in range(10)]

    def fmt_row(r, last_row):
        last_non_empty = max(c for c in range(10) if grid[r][c] != "")
        parts = []
        for c in range(10):
            key = grid[r][c]
            w = cw[c]
...
def format_mirrored_layer(name, grid):
    """Format a mirrored layer. Alpha rows: 5 keys (cols 0-4), cols 5-9 empty.
    Thumb row: 10 cols with "" at 0,1,8,9. Same column alignment as full layers."""
    full_grid = [grid[r] + [""] * 5 for r in range(3)] + [grid[3]]
    cw = [max(len(full_grid[r][c]) for r in range(4)) for c in range(10)]

    def fmt_row(r, last_row):
        last_non_empty = max(c for c in range(10) if full_grid[r][c] != "")
        parts = []
        for c in range(10):
            key = full_grid[r][c]
            w = cw[c]
...
# ── Layer definitions ─────────────────────────────────────────────────────
# Derived from Totem ZMK layout, adapted for 3w6 (no side keys).
...
if __name__ == "__main__":
    ...
    for i, (name, ltype, grid) in enumerate(layers):
        if ltype == "mirrored":
            output += format_mirrored_layer(name, grid)
        else:
            output += format_layer(name, "LAYOUT_split_3x5_3", grid)
        if i < len(layers) - 1:
            output += "\n"
        output += "\n"
